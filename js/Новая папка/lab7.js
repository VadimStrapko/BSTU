...
//для каждого уровня вложенности
  for(let item in products.Shoes){
      ...
      //для каждого эдемента одного уровня
      shoes.forEach(shoe => {
...
// свойство-аксессор
//для получения массива ключей категорий обуви
Object.keys(newProducts.Shoes).forEach(category =>{
  newProducts.Shoes[category].forEach(shoe => {
      shoe.cost = shoe.price;
      Object.defineProperty(shoe, 'id', {// Добавляет свойство 'id' для каждой обуви с определенными характеристиками
          writable: false, // нельзя изменять
          enumerable: true,//будет видимым при переборе
          configurable: false // нельзя удалять 
      });
      //удадаление первого эл-та
...
function shoesPare(id, size, color, discount, cost){
  ...
  Object.defineProperty(this, 'price', {
      ...
      set(newPrice){
          this.cost = newPrice//устанавливает новую цену
      },
      enumerable:true,
      configurable: false
  });
...
//работа скидки
let someShoe = new shoesPare(11, 35, 'red', 10, 100);
console.log(someShoe.price); //выведет 90(100 - 10% от 100)

// 6 ЗАДАНИЕ
//создание объектов конструктора  ////////////////////////////////////////////////
let allProduct={
  Shoes:{
      Boots:[
